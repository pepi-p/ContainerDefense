...
public class PlayerMove : MonoBehaviour
{
    ...
    public static float angleSpeed = 2;
    ...
    [SerializeField] private GameObject torso2;
    ...
    [Header("Scripts")]
    [SerializeField] private manager _maneger;
    [Header("SE")]
    ...
    [SerializeField] private AudioClip emptyShotSE;
    ...
    private float rotationX, rotationY, recoil; // X軸回転(視点上下) Y軸回転(視点左右) X軸回転の反動分
    ...
    private void Update()
    {
        ...
        var mouseX = !_maneger.stop ? Input.GetAxis("Mouse X") : 0;
        var mouseY = !_maneger.stop ? Input.GetAxis("Mouse Y") : 0;

        var aim = !_maneger.stop ? Input.GetMouseButton(1) : false;
        var shot = !_maneger.stop ? Input.GetMouseButton(0) : false;

        rotationY += !_maneger.stop ? mouseX * angleSpeed : 0;
        rotationX += !_maneger.stop ? mouseY * angleSpeed : 0;
        if(aim) {
        ...
        if(Input.GetMouseButtonDown(1)) audioSourceNormal.PlayOneShot(kamaeSE);
        ...
        Light.transform.rotation = Quaternion.Euler(50, -30 + rotationY, 0);
        ...
        playerAnimation.IKWeight = torsoWeight;
        // playerAnimation.angle = rotationX + recoil <= 15 ? (rotationX + recoil) * 0.8f : 6.2f * Mathf.Sqrt(rotationX + recoil) - 12;
        if(rotationX + recoil > 15) playerAnimation.angle = 6.2f * Mathf.Sqrt(rotationX + recoil) - 12;
        else if(rotationX + recoil < -25) playerAnimation.angle = -(1 / 38.44f) * Mathf.Pow(rotationX + recoil + 9.624f, 2) -13.8496f;
        else playerAnimation.angle = (rotationX + recoil) * 0.8f;

        if(torsoWeight > 0.5f) {
            mainCamera.transform.parent = torso2.transform;
            ...
            pos.y = this.transform.position.y + 1.579207f;
            ...
            torso2.transform.rotation = Torso.transform.rotation;
        }
        else if(isHit) {
        ...
        // reload
        if(Input.GetKeyDown(KeyCode.R) && !reload) StartCoroutine("Reload");

        // shot
        if(shot && ammo > 0 && !reload && torsoWeight > 0.9f && coolDown > 0.0824f) {
            coolDown = 0;
            Shot();
        }
        coolDown += Time.deltaTime;
        if(Input.GetMouseButtonDown(0) && ammo <= 0) audioSourceNormal.PlayOneShot(emptyShotSE);
    ...
    private void Shot()
    {
        ...
        if(rayhit.collider.CompareTag("Enemy")) {
            rayhit.collider.GetComponent<EnemyController>().Hit(rayhit.point);
            if(rayhit.point.y - rayhit.collider.gameObject.transform.position.y > 1.5f) _maneger.headShotCount++;
        }
        /*
        if(rayhit.collider.CompareTag("container")) {
            rayhit.collider.GetComponent<Container>().HP -= 0.01f;
        }
        */
